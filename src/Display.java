...
public class Display {
    ...
    public void displayBoard(List<List<Square>> board){
        ...
        for (int col = 0; col < board.toArray().length; col++) {
            ...
            for (int row = 0; row < board.toArray().length; row++) {
                if (board.get(col).get(row).getStatusSquare().equals("S")){                      //jeśli jest statek
                    boardString += ansiColors.ANSI_BLACK_BACKGROUND + "   "  + ansiColors.ANSI_RESET;
                }else if (board.get(col).get(row).getStatusSquare().equals("H")){                //jeśli statek częściowo jest podbity
                    boardString += ansiColors.ANSI_BLACK_BACKGROUND + ansiColors.ANSI_RED + " ● "  + ansiColors.ANSI_RESET;
                }else if (board.get(col).get(row).getStatusSquare().equals("D")){                //jeśli statek jest cały zniszczony
                    boardString += ansiColors.ANSI_BLACK_BACKGROUND + ansiColors.ANSI_RED + " X "  + ansiColors.ANSI_RESET;
                }else if (board.get(col).get(row).getStatusSquare().equals("M")){                //jeśli był strzał w puste miejsce
